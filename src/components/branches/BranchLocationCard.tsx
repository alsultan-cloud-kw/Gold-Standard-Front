  ...
  const details = (
    <>
      <div className="flex w-full items-center gap-3">
        <span
          className={cn(
            'flex shrink-0 items-center justify-center rounded-xl font-mono text-xs font-bold',
            compact ? 'h-10 w-10' : 'h-11 w-11',
            selected ? 'bg-[#0B0F19] text-[#85E307]' : 'bg-[#F1F5F9] text-[#475569]',
          )}
        >
          {branchMarkerLabel()}
        </span>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex w-full flex-wrap items-center gap-x-2 gap-y-1">
            <h2 className={cn('type-card-title text-[#0B0F19]', compact && 'text-sm')}>{name}</h2>
      ...
      <div
        className={cn(
          'mt-4 w-full space-y-3.5 rounded-xl border border-black/6 bg-white/70 text-sm text-[#475569]',
          compact ? 'space-y-3 p-3' : 'space-y-3.5 p-3.5 sm:p-4',
    ...
    >
      ...
      <div
        className={cn(
          ...
          solo && !compact && 'min-h-[min(52vh,380px)] sm:min-h-[min(56vh,440px)] lg:min-h-[min(68vh,560px)]',
        )}
      >
        {readOnly ? (
          <div className="w-full flex-1 text-start">{details}</div>
        ) : (
          <button
            type="button"
            onClick={onSelect}
        ...
        <a
          ...
          className={cn(
            'mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-black/10 bg-white font-semibold text-[#0B0F19] shadow-sm transition-colors hover:border-[#85E307]/40 hover:bg-[#ECFCCB]/50',
