...
import { Users, ShoppingBag, DollarSign, AlertTriangle, ArrowUpRight, ArrowDownRight } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const { data: recentOrdersRaw, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: () =>
      allTime
        ? ordersApi.getOrders({ page_size: '10', ordering: '-sale_date' })
        : ordersApi.getOrders({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          ...
          <div className="text-right sm:text-end">
            <p className="text-sm text-gold-100/60">{new Date().toLocaleDateString()}</p>
          </div>
        </div>
        ...
        <div className="gold-card mb-8">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <p className="text-sm text-gold-400 font-mono">{periodDescription}</p>
            </div>
            ...
            {allTime && (
              <p className="text-xs text-gold-100/50">{t('admin.dashboardThroughToday')} — {localISODate(todayDate())}</p>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {kpiCards.map((card, index) => (
            <div key={index} className="gold-card">
              ...
              <h3 className="text-2xl font-bold text-gold-100">{card.value}</h3>
              <p className="text-sm text-gold-100/60">{t(card.titleKey)}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="gold-card">
            <h2 className="text-xl font-bold text-gold-100 mb-4">{t('admin.recentOrdersInRange')}</h2>
            {ordersLoading ? (
              <p className="text-gold-100/60 text-sm">{t('common.loading')}</p>
            ) : recentOrders.length === 0 ? (
              <p className="text-gold-100/60 text-sm">{t('admin.noOrdersFound')}</p>
            ) : (
              <div className="space-y-3">
                {recentOrders.map((order) => (
                  <div
                    key={order.id}
                    className="flex items-center justify-between p-3 bg-charcoal-800 rounded-lg"
                  >
                    <div>
          ...
          <div className="gold-card">
            <h2 className="text-xl font-bold text-gold-100 mb-4">{t('admin.lowStockAlerts')}</h2>
            <p className="text-xs text-gold-100/50 mb-3">{t('admin.lowStockSnapshotNote')}</p>
            {lowStockLoading ? (
              <p className="text-gold-100/60 text-sm">{t('common.loading')}</p>
            ) : lowStockItems.length === 0 ? (
              <p className="text-gold-100/60 text-sm">{t('admin.noLowStockItems')}</p>
            ) : (
              <div className="space-y-3">
