...
function applyPageMeta(html, page) {
  ...
  // Prefer English titles for OG/WhatsApp (broad scrapers); bilingual description.
  const title = page.titleEn
  const description = `${page.descAr} | ${page.descEn}`
  ...
  out = replaceOrInsertProperty(out, 'og:description', page.descEn)
  ...
  out = replaceOrInsertName(out, 'twitter:description', page.descEn)
...
function writeSitemap() {
  const urls = PRERENDER_PAGES.map((page) => {
